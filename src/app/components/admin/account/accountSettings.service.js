/* Action about admin Account  */
  angular.module('cupid').factory('accountSettingAction', ['$rootScope', '$q' , 'global' , '$http','common', function($rootScope,$q,global,$http,common) {
    var account = {
       ...
       * @param email {string} 需要更新的邮件名
       * @param name {string} 需要更新的账户名称
       * @param token {string} 通行秘钥
       * @returns Object
       * @date 2016-04-07
       * @author weilewantieba@126.com
       * @todo 分页没有处理.有两个参数没有去解决
       */
      updateAccount: function (userType, platformType, userId, token ,accId ,email , name , desc) {
        var deferred = $q.defer(); // 声明延后执行，表示要去监控后面的执行
        var parameter = {
          method: 'POST',
          url: global.localUrl + '/admin/account/update',
          data: {
            "ws": {
              "userType": userType,
              "platformType": platformType,
